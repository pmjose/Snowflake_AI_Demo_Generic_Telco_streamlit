...
-- Loads all data from stage into tables
-- Run time: ~3-5 minutes
-- Prerequisites: Run 01-03 scripts first
-- ========================================================================
...
SELECT '    This may take 3-5 minutes...' AS note;
    ...
    COPY INTO sf_quotas
    FROM @INTERNAL_DATA_STAGE/demo_data/sf_quotas.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Salesforce Pipeline Snapshots
    COPY INTO sf_pipeline_snapshot
    FROM @INTERNAL_DATA_STAGE/demo_data/sf_pipeline_snapshot.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Salesforce Open Pipeline (2026 opportunities)
    COPY INTO sf_opportunities
    FROM @INTERNAL_DATA_STAGE/demo_data/sf_opportunities_open.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/mobile_plan_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Mobile Device Dimension
    COPY INTO mobile_device_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/mobile_device_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Mobile Network Dimension
    COPY INTO mobile_network_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/mobile_network_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Mobile Subscriber Dimension
    COPY INTO mobile_subscriber_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/mobile_subscriber_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Mobile Usage Fact
    COPY INTO mobile_usage_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/mobile_usage_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM (
        ...
        FROM @INTERNAL_DATA_STAGE/demo_data/mobile_churn_fact.csv
    )
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/support_category_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Order Dimension
    COPY INTO order_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/order_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Order Line Fact
    COPY INTO order_line_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/order_line_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Invoice Fact
    COPY INTO invoice_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/invoice_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Service Instance Fact
    COPY INTO service_instance_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/service_instance_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Support Ticket Fact
    COPY INTO support_ticket_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/support_ticket_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Network Element Dimension
    COPY INTO network_element_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/network_element_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/ran_site_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load RAN Equipment Dimension
    COPY INTO ran_equipment_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/ran_equipment_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load RAN Cell Dimension
    COPY INTO ran_cell_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/ran_cell_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load RAN Performance Fact
    COPY INTO ran_performance_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/ran_performance_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load RAN Alarm Fact
    COPY INTO ran_alarm_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/ran_alarm_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/competitor_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Competitor Pricing Dimension
    COPY INTO competitor_pricing_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/competitor_pricing_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Technician Dimension
    COPY INTO technician_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/technician_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Customer Journey Fact
    COPY INTO customer_journey_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/customer_journey_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Network QoE Fact
    COPY INTO network_qoe_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/network_qoe_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Customer Propensity Scores
    COPY INTO customer_propensity_scores
    FROM @INTERNAL_DATA_STAGE/demo_data/customer_propensity_scores.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Field Visit Fact
    COPY INTO field_visit_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/field_visit_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Market Share Fact
    COPY INTO market_share_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/market_share_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Social Mention Fact
    COPY INTO social_mention_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/social_mention_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Energy Consumption Fact
    COPY INTO energy_consumption_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/energy_consumption_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Sustainability Metrics
    COPY INTO sustainability_metrics
    FROM @INTERNAL_DATA_STAGE/demo_data/sustainability_metrics.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/fraud_type_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Fraud Case Fact
    COPY INTO fraud_case_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/fraud_case_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/b2b_contract_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/mvno_partner_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load MVNO Traffic Fact
    COPY INTO mvno_traffic_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/mvno_traffic_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load MVNO Settlement Fact
    COPY INTO mvno_settlement_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/mvno_settlement_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/retail_store_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Retail Sales Fact
    COPY INTO retail_sales_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/retail_sales_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Retail Footfall Fact
    COPY INTO retail_footfall_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/retail_footfall_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/employee_detail_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Employee Survey Fact
    COPY INTO employee_survey_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/employee_survey_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Network Alarm Fact
    COPY INTO network_alarm_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/network_alarm_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Network Performance Fact
    COPY INTO network_performance_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/network_performance_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/partner_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM (
        SELECT 
            ...
            $12
        FROM @INTERNAL_DATA_STAGE/demo_data/partner_performance_fact.csv
    )
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/contact_center_agent_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Contact Center Call Fact (split files)
    COPY INTO contact_center_call_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/contact_center_call_fact_1.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    COPY INTO contact_center_call_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/contact_center_call_fact_2.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Payment Method Dimension
    COPY INTO payment_method_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/payment_method_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Payment Fact
    COPY INTO payment_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/payment_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Warehouse Dimension
    COPY INTO warehouse_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/warehouse_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Inventory Fact
    COPY INTO inventory_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/inventory_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Purchase Order Fact
    COPY INTO purchase_order_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/purchase_order_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Purchase Order Line Fact
    COPY INTO purchase_order_line_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/purchase_order_line_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Fixed Asset Dimension
    COPY INTO fixed_asset_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/fixed_asset_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Contract Dimension
    COPY INTO contract_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/contract_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    -- Load SLA Dimension
    COPY INTO sla_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/sla_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load SLA Measurement Fact
    COPY INTO sla_measurement_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/sla_measurement_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Roaming Partner Dimension
    COPY INTO roaming_partner_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/roaming_partner_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Roaming Usage Fact
    COPY INTO roaming_usage_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/roaming_usage_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Number Range Dimension
    COPY INTO number_range_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/number_range_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Number Port Fact
    COPY INTO number_port_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/number_port_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load IT Application Dimension
    COPY INTO it_application_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/it_application_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load IT Incident Fact
    COPY INTO it_incident_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/it_incident_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Complaint Fact
    COPY INTO complaint_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/complaint_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Legal Matter Fact
    COPY INTO legal_matter_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/legal_matter_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    ...
    FROM @INTERNAL_DATA_STAGE/demo_data/loyalty_program_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Loyalty Transaction Fact
    COPY INTO loyalty_transaction_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/loyalty_transaction_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load IoT Device Type Dimension
    COPY INTO iot_device_type_dim
    FROM @INTERNAL_DATA_STAGE/demo_data/iot_device_type_dim.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load IoT Subscription Fact
    COPY INTO iot_subscription_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/iot_subscription_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load IoT Usage Fact
    COPY INTO iot_usage_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/iot_usage_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Digital Interaction Fact (split files)
    COPY INTO digital_interaction_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/digital_interaction_fact_1.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
    
    COPY INTO digital_interaction_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/digital_interaction_fact_2.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- ========================================================================
    -- LOAD NEW ENHANCEMENT TABLES FROM INTERNAL STAGE
    -- ========================================================================

    -- Load Credit Note Fact (Revenue Assurance)
    COPY INTO credit_note_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/credit_note_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Billing Adjustment Fact (Revenue Assurance)
    COPY INTO billing_adjustment_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/billing_adjustment_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Unbilled Usage Fact (Revenue Leakage)
    COPY INTO unbilled_usage_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/unbilled_usage_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load SIM Activation Fact (Operations)
    COPY INTO sim_activation_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/sim_activation_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load Dispute Fact (Billing)
    COPY INTO dispute_fact
    FROM @INTERNAL_DATA_STAGE/demo_data/dispute_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';
...
SELECT 
    'mobile_subscriber_dim' AS table_name, COUNT(*) AS row_count FROM mobile_subscriber_dim
...
SELECT 'Next step: Run 05_semantic_views.sql' AS next_step;
